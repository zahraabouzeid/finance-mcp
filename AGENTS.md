...
Rules for AI agents working in this repository. They apply to every agent and every tool.

Project decisions such as architecture, security, testing and technology choices are documented in `openspec/`, not in this file.
...
Changes follow [OpenSpec](https://github.com/Fission-AI/OpenSpec). The workflows are in `.agents/skills/`. Read the matching `SKILL.md` before starting one.

- Every OpenSpec change is one branch and one pull request.
- If a change is too big for one pull request, split it into smaller changes.
- Changes that do not alter behavior, such as typos, dependency updates, formatting or CI, need no OpenSpec change. They still go through a pull request.
- Order: propose, review, apply, validate, archive, merge.
- Archive only when all tasks are done and all checks pass.
- Archive the change in the same pull request, before the merge.
- Archived changes are never edited. To change a decision, open a new change.
- Files in `.agents/skills/` are generated by OpenSpec. Do not edit them by hand. Refresh them with `openspec update`.
